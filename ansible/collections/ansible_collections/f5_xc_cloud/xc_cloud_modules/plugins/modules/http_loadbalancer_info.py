...
DOCUMENTATION = r'''
---
module: http_loadbalancer_info
short_description: Gather information about F5 Distributed Cloud HTTP Load Balancers
description:
    - Retrieve information about F5 Distributed Cloud HTTP Load Balancers
    - This is a read-only module that does not modify any resources
    - Supports querying single load balancers or listing all in a namespace
    - Returns detailed configuration and status information
version_added: "0.0.1"
options:
    name:
        description:
            - Name of a specific HTTP Load Balancer to retrieve
            - If not specified, all load balancers in the namespace will be returned
    ...
    filters:
        description:
            - Optional filters to apply when querying multiple load balancers
        ...
        suboptions:
            labels:
                description:
                    ...
                    - Key-value pairs that must match load balancer labels
                type: dict
            state:
                description:
                    - Filter by load balancer state
                type: str
                choices: ['ACTIVE', 'PENDING', 'ERROR', 'DELETING']
            domains:
                description:
                    - Filter load balancers that serve specific domains
...
- name: Get specific load balancer info
  http_loadbalancer_info:
    name: "my-load-balancer"
    namespace: "production"
  register: lb_info

- name: Display load balancer domains
  debug:
    msg: "Load balancer serves: {{ lb_info.resources[0].spec.domains }}"

# Get all load balancers in a namespace
- name: Get all load balancers in namespace
  http_loadbalancer_info:
    namespace: "production"
  register: all_lbs

- name: Show count of load balancers
  debug:
    msg: "Found {{ all_lbs.resources | length }} load balancers"

# Filter load balancers by labels
- name: Get load balancers with specific labels
  http_loadbalancer_info:
    namespace: "production"
    filters:
      labels:
        environment: "prod"
        team: "platform"
  register: filtered_lbs

# Filter by domain
- name: Get load balancers serving specific domains
  http_loadbalancer_info:
    namespace: "production"
    filters:
      domains:
...
- name: Get load balancer metadata only
  http_loadbalancer_info:
    name: "my-load-balancer"
    namespace: "production"
    include_spec: false
    include_status: false
  register: lb_metadata

# Filter by state
- name: Get only active load balancers
  http_loadbalancer_info:
    namespace: "production"
    filters:
      state: "ACTIVE"
...
- name: Get load balancers from multiple namespaces
  http_loadbalancer_info:
    namespace: "{{ item }}"
  loop:
    - "production"
...
- name: Fail if load balancer doesn't exist
  fail:
    msg: "Load balancer 'my-load-balancer' not found"
  when: lb_check.resources | length == 0

# Get load balancers and extract specific information
- name: Get load balancers and extract WAF policies
  http_loadbalancer_info:
    namespace: "production"
  register: lbs

- name: List WAF policies in use
  debug:
    msg: "WAF Policy: {{ item.spec.app_firewall.name }}"
  loop: "{{ lbs.resources }}"
  when: item.spec.app_firewall is defined
'''
...
changed:
    ...
    sample: false
resources:
    description: List of HTTP Load Balancer resources
    type: list
    returned: always
    elements: dict
    contains:
        metadata:
            ...
            contains:
                ...
                disable:
                    ...
                    sample: false
        spec:
            description: Load balancer specification (when include_spec=true)
            type: dict
            contains:
                domains:
                ...
                app_firewall:
                    ...
                    type: dict
        status:
            description: Load balancer status (when include_status=true)
            type: dict
            contains:
                state:
...
from ..module_utils.constants import HTTP_LOADBALANCERS_ENDPOINT
...
class InfoParameters(AnsibleF5Parameters):
    ...
    def _validate_filters(self):
        ...
        if 'domains' in filters:
            if not isinstance(filters['domains'], list):
                raise XcValidationError("domains filter must be a list")
...
class InfoManager(object):
    ...
    def _filter_resource(self, resource):
        ...
        # Filter by domains
        if 'domains' in filters:
            resource_domains = safe_get(resource, 'spec', 'domains', default=[])
            filter_domains = filters['domains']
            
            if self.want.exact:
                # Exact match: resource must serve all specified domains
                if not all(domain in resource_domains for domain in filter_domains):
                    return False
            else:
                # Partial match: resource must serve at least one specified domain
                if not any(
                    any(filter_domain in resource_domain for resource_domain in resource_domains)
    ...
    # Note: Some HTTP load balancers may not expose a status block; absence is not an error.
    ...
    def get_single_resource(self):
        """Get information about a single load balancer."""
        try:
            ...
            if response_data and response_data.get('metadata'):
                ...
                self.resources = []
                self.warnings.append(f"Load balancer '{self.want.name}' not found in namespace '{self.want.namespace}'")
    ...
    def get_multiple_resources(self):
        """Get information about multiple load balancers."""
        try:
            uri = self._build_uri()
            response = self.client.api.get(url=uri)
            ...
            if response_data and 'items' in response_data:
                ...
                self.resources = []
                self.warnings.append(f"No load balancers found in namespace '{self.want.namespace}'")
                
        except XcApiError as e:
            if e.status_code == 404:
                self.resources = []
                self.warnings.append(f"Namespace '{self.want.namespace}' not found or no load balancers exist")
            else:
                raise
